...
% Assuming that genotypeFactor contains only a single var and a single integer card
for k = 1:genotypeFactor.card, 
	i = genotypesToAlleles(k,:)(1,1);
	j = genotypesToAlleles(k,:)(1,2);
	if i==j, 
		genotypeFactor.val(1, k) = alleleFreqs(i) * alleleFreqs(j);
	else
		genotypeFactor.val(1, k) = 2 * alleleFreqs(i) * alleleFreqs(j);
	end;
end;
