...
for i=1:numAlleles,
	for j=1:numAlleles,
		for d=1:2,
			% each phenotype assignment (aka 1 has the trait, 2 doesn't have the trait)
			alphaIndex = 0;
			for k=1:m,
				thisGenotype = genotypesToAlleles(k,:);
				if ([i j] == thisGenotype || [j i] == thisGenotype),
					alphaIndex = k;
				end;
			end
			if d==1,
				phenotypeFactor.val(phenoValCounter) = alphaList(alphaIndex);
			else
				phenotypeFactor.val(phenoValCounter) = 1 - alphaList(alphaIndex);
			end;
			phenoValCounter = phenoValCounter + 1;
		end
	end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%