...
genotypeFactorPar2 = struct('var', [3,1,2], 'card', [3,3,3], 'val', [1,0,0,0.5,0.5,0,0,1,0,0.5,0.5,0,0.25,0.5,0.25,0,0.5,0.5,0,1,0,0,0.5,0.5,0,0,1]); % Comment out this line for testing
...
phenotypeVar = 3;
phenotypeFactorPar2 = struct('var', [3,1,2], 'card', [2,3,3], 'val', [0.8,0.2,0.6,0.4,0.1,0.9,0.6,0.4,0.5,0.5,0.05,0.95,0.1,0.9,0.05,0.95,0.01,0.99]); % Comment out this line for testing
...
sampleFactorListDecoupled.sampleFactorListDecoupled(1) % the sample data structure is not in the right shape
X = constructDecoupledGeneticNetwork(pedigree, alleleFreqsThree, alphaListThree);
...
X = constructSigmoidPhenotypeFactor(alleleWeights, geneCopyVarParentOneList, geneCopyVarParentTwoList, phenotypeVar);
% Note: my solutions indexes the permutations differently than the sample solution, but is otherwise sound